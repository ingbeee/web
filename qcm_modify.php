...
if (isset($_GET['commentID']) && isset($_GET['boardID'])) {
    ...
    $query = "SELECT userID, content FROM reply1 WHERE idx = ?";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['content'])) {
    ...
    if (empty($newContent)) {
        ...
        $conn = mysqli_connect('localhost', 'root', '8963', 'BOARD') or die("데이터베이스 연결 실패");
        $query = "UPDATE reply1 SET content = ? WHERE idx = ?";
        $stmt = mysqli_prepare($conn, $query);
