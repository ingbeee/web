...
if (isset($_POST['comment_id']) && isset($_POST['boardID'])) {
    ...
    $query = "DELETE FROM reply1 WHERE idx = ?";
    $stmt = mysqli_prepare($conn, $query);
...
<body>
    <div class="comment-container">
        ...
        function getComments($boardID) {
            ...
            $query = "SELECT idx, userID, content, date, boardID, file, link FROM reply1  WHERE boardID = ?";
            $stmt = mysqli_prepare($conn, $query);
...
?><br>
            <tr><td class="view_id">파일</td>
                <td class="view_id2">
                    ...
                    <a href="download.php?board=qcm&number=<?php echo $comment['idx']; ?>"><?php echo $comment['file']?></a>
                    <?php else: ?>
                        파일 없음<?php endif; ?></td>
                    </tr>
            ...
            if (isset($_SESSION['userID']) && ($_SESSION['userID'] === $comment['userID'] || $_SESSION['userID'] === $comment['boardID'] || $isAdmin)) {
    // 댓글 작성자와 로그인 사용자가 일치하는 경우에만 수정 버튼 표시
    if ($_SESSION['userID'] === $comment['userID']) {
        if ($comment['content'] !== '삭제된 댓글입니다.') {
            echo '<form style="display: inline-block;" method="get" action="qcm_modify.php">';
   ...
   if ($comment['content'] !== '삭제된 댓글입니다.') {
    echo '<form style="display: inline-block;" method="post" action="qcm_delete.php">';
    ...
    <div class="comment-form">
        <h2>답글 작성</h2>
        <!-- 댓글 작성 폼 -->
        <form method="post" action="qcm_write_ok.php?idx=<?php echo $number ?>"enctype="multipart/form-data">
    ...
    <form method="get" action="q_read.php">
