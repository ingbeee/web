...
// 조회수 업데이트 쿼리 실행
$hit_update_query = "UPDATE qna SET hit = hit + 1 WHERE number = $number";
$connect->query($hit_update_query);
...
// 게시물 정보 가져오기
$query = "SELECT title, content, date, ID, secret, hit, file, link FROM qna WHERE number = ?";
...
<table class="view_table" align="center">
    ...
    <tr><td class="view_id">파일</td>
                <td class="view_id2">
                    ...
                    <a href="download.php?board=qna&number=<?php echo $number; ?>"><?php echo $rows['file']?></a>
                    <?php else: ?>
                        파일 없음<?php endif; ?></td>
                    </tr>
...
<button class="view_btn1" onclick="location.href='./QnA.php'">목록으로</button>
    <?php if ($_SESSION['userID']): ?>
        ...
        <?php if (($_SESSION['userID'] === $rows['ID']) && $can_delete_mod): ?>
         ...
         <a href="q_modify.php?idx=<?php echo $number; ?>">수정</a>
         </button>
        <?php endif; ?>
    ...
    <?php if (($_SESSION['userID'] === $rows['ID']) && $can_delete_mod): ?>
        <button class="view_btn1">
         <a href="q_delete.php?idx=<?php echo $number; ?>">삭제</a>
         </button>
    <?php elseif ($userRoll === 'admin' && $can_delete_mod): ?>
        <button class="view_btn1">
        <a href="qdelete.php?idx=<?php echo $number; ?>">삭제</a>
         </button>
    <?php endif; ?>

        <button class="view_btn1">
         <a href="qcm.php?idx=<?php echo $number; ?>">답글</a>
         </button>
